...
class CompileEngine(object):
    ...
    def _CompileSubroutineBody(self):
        ...
        # Now we know how many locals there are
        nLocals = self.symbolTable.VarCount(SYMK_VAR)
        functionName = self.className + '.' + self.subroutineName
        self.vmWriter.WriteFunction(functionName, nLocals) # use the vmWriter to write functino declaraion

        # Setup code depends on subroutine type
        if self.subroutineType == KW_METHOD:
    ...
    def _CompileCall(self, subroutineName=None):
        ...
        if self.tokenizer.TokenType() == TK_SYMBOL and \
            ...
            if variableKind is not None:
                # object.method() — push the object as argument 0
                variableIndex = self.symbolTable.IndexOf(objectName)
                variableSegment = self._KindToSegment(variableKind)
                self.vmWriter.WritePush(variableSegment, variableIndex)
                # use the object's type as class name
                className = self.symbolTable.TypeOf(objectName)
                nArgs = 1
            else:
                # Class.function() — no object to push
                className = objectName

            fullName = className + '.' + subroutineName

        else:
            # name(args) — method on this
            self._WriteXml('identifier', subroutineName)
            self.vmWriter.WritePush(SEG_POINTER, 0)
            nArgs = 1
            fullName = self.className + '.' + subroutineName
